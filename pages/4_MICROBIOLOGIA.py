...
if gid_actual == "PONER_GID_AQUI":
    ...
    if isinstance(df_limpio, str):
        ...
        if etapa_seleccionada == "1. Control de Propagación":
            ...
            if col_ft:
                df_limpio[col_ft] = df_limpio[col_ft].fillna("").astype(str).str.strip().str.upper()
                df_valid_ft = df_limpio[~df_limpio[col_ft].isin(["", "NAN", "NONE", "N/A"])]
                lista_fts = ["Todos los FT"] + list(df_valid_ft[col_ft].unique())
            ...
            if cols_graficables:
                ...
                for fase_tipo in fases_presentes:
                    ...
                    for idx, row in group.iterrows():
                        ...
                        lote_txt = row.get('Propagacion', row.get(col_ft, 'N/A'))
