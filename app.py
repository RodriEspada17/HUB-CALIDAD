...
# =====================================================================
# 📚 DICCIONARIO MAESTRO: (TOL_MIN, TOL_MAX, STD_MIN, STD_MAX)
# =====================================================================

SPECS_CCR_REPOSO = {
    "EXTRACTO": (14.0, 15.0, 14.1, 14.9), "EO": (14.0, 15.0, 14.1, 14.9), 
    "EA": (2.70, 3.30, 2.80, 3.20),
    "ALCOHOL": (5.80, 6.40, 5.90, 6.30), "ALC": (5.80, 6.40, 5.90, 6.30),
    "COLOR": (5.90, 8.90, 6.40, 8.40), "AMARGO": (9.50, 13.50, 10.00, 13.00), 
    "PH": (3.95, 4.45, 4.05, 4.35), "TURBIDEZ": (0, 30.0), "DIACETILO": (0, 100.0)
}
...
ESPECIFICACIONES = {
    "COCIMIENTO_REPOSO": {
        ...
        "MALTA REAL": SPECS_MALTA_REPOSO, "MALTA": SPECS_MALTA_REPOSO,
        "CAPITAL": SPECS_CCR_REPOSO, "CORDILLERA": SPECS_CCR_REPOSO, "REAL": SPECS_CCR_REPOSO
    },
    "FILTRACION_PT": {
        "AMSTEL": {
        ...
        "MALTA REAL": SPECS_MALTA_PT, "MALTA": SPECS_MALTA_PT,
        "CAPITAL": SPECS_CCR_PT, "CORDILLERA": SPECS_CCR_PT, "REAL": SPECS_CCR_PT
    }
}
...
def obtener_limites(etapa, producto, parametro):
    diccionario_etapa = ESPECIFICACIONES["COCIMIENTO_REPOSO"] if etapa in ["Cocimiento", "Fin de Reposo"] else ESPECIFICACIONES["FILTRACION_PT"]
...
if df is not None and not df.empty:
    ...
    def pintar_celdas(val, lsl, usl):
        try:
            ...
            if lsl is not None and usl is not None:
                if v < lsl or v > usl: return 'background-color: #ffcccc; color: #990000;'
                else: return 'background-color: #ccffcc; color: #006600;'
        except: pass
        return ''
    ...
    def aplicar_semaforo(row):
        ...
        # Identificar el producto fila por fila si estamos en la vista "Todos"
        prod_fila = producto_actual
        if prod_fila == "Todos":
            col_p = next((c for c in row.index if "PRODUCTO" in str(c).upper()), None)
            if col_p:
                prod_fila = str(row[col_p])
            else:
                return estilos # Sin saber el producto, no podemos aplicar color
                
        for i, col in enumerate(row.index):
            lsl, usl, _, _ = obtener_limites(etapa_seleccionada, prod_fila, col)
            if lsl is not None: 
    ...
    with tab_datos:
        ...
        # Ahora el apply(aplicar_semaforo) siempre se ejecuta, incluso con "Todos"
        st.dataframe(df.style.apply(aplicar_semaforo, axis=1), use_container_width=True)
        st.caption("🟢 Verde: Dentro de Tolerancia (TOL) | 🔴 Rojo: Fuera de Tolerancia")
    ...
    with tab_spc:
        ...
        if cols_num:
            ...
            st.caption("💡 *Las casillas de LSL y USL se autocompletan solas con la Tolerancia, pero puedes escribirlas a mano si deseas hacer simulaciones y ver qué pasaría con tu Cpk.*")
            ...
            with col_a:
                lsl = st.number_input("LSL (Límite Inferior TOL):", value=float(lsl_auto if lsl_auto is not None else datos_clean.min()))
            with col_b:
                usl = st.number_input("USL (Límite Superior TOL):", value=float(usl_auto if usl_auto is not None else datos_clean.max()))
        ...
        if cols_num:
            ...
            if len(df_trend) > 0:
                ...
                if lsl_auto is not None and usl_auto is not None:
                    ...
                    st.info("Sin especificación manual. Mostrando +/- 3 Sigma.")
